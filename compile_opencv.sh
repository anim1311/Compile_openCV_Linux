...
sudo apt-get -y upgrade       # comment to not install new versions of packages currently installed
sudo apt-get -y dist-upgrade  # comment to not handle changing dependencies with new vers. of pack.
sudo apt-get -y autoremove    # comment to not remove packages that are now no longer needed
